(function(){
  ...
  var STYLE='\n'
    ...
    + '.scw-name-edit input{flex:1 1 100%;min-width:0;padding:4px 6px;border-radius:6px;border:1px solid '+COLORS.border+';font:12px sans-serif;background:'+COLORS.bgDefault+';color:'+COLORS.textPrimary+'}\n'
  ...
  function style(){ if(__scwStyleInjected) return; var s=el('style'); s.textContent=STYLE; d.head.appendChild(s); __scwStyleInjected=true; }
...
function connect(origin,token,onMsg,onOpen,onClose){var ws=new WebSocket(origin.replace(/^http/,'ws')+'/v1/ws?token='+encodeURIComponent(token));ws.onmessage=function(e){try{var m=JSON.parse(e.data);if(m&&m.type==='agent_joined'){var SS=(window.SupportChat&&window.SupportChat.__strings)||{supportLabel:'Support',joinedSuffix:' joined'}; var who=(m.agent&&String(m.agent).trim().length)?m.agent:(SS.supportLabel||'Support'); var txt=who + (SS.joinedSuffix||' joined'); onMsg('SYS', txt, Date.now(), null, true);return;} if(m&&m.type==='info_note'){ onMsg('SYS', (m.text||''), Date.now()); return; } if(m&&typeof m.text==='string'){onMsg(m.direction==='OUTBOUND'?'OUT':'IN',m.text,Date.now(),m.agent)}}catch(_){} };ws.onopen=function(){onOpen&&onOpen()};ws.onclose=function(){onClose&&onClose()};ws.onerror=function(){try{ws.close()}catch(_){}};return ws}
  ...
  function init(opts){style();var origin=((opts&&opts.origin)||detectOrigin()).replace(/\/$/,'');var store=w.localStorage||{getItem:function(){},setItem:function(){}};
    ...
    var STR = (function(){ var base = {}; for(var k in STR_DEFAULT){ base[k]=STR_DEFAULT[k]; }
      ...
      return base; })();
    try{ w.SupportChat = w.SupportChat || {}; w.SupportChat.__strings = STR; }catch(_){ }
