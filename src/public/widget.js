(function(){
  ...
  function init(opts){style();var origin=((opts.origin||'')||detectOrigin()).replace(/\/$/,'');var store=w.localStorage||{getItem:function(){},setItem:function(){}};
    ...
    function loadHistory(){ var id=store.getItem('scw:id'); if(!id) return Promise.resolve(); return fetch(origin + '/v1/conversations/' + encodeURIComponent(id) + '/messages').then(function(r){ if(!r.ok) throw new Error(); return r.json() }).then(function(payload){ var status=payload.status; var msgs=payload.messages||[]; body.innerHTML=''; msgs.forEach(function(m){ addMsg(body, m.direction==='INBOUND'?'IN':'OUT', m.text, new Date(m.createdAt).getTime(), m.agent) }); if(status==='CLOSED'){ addSystem(body,'Conversation closed. Start new?', Date.now()); } }).catch(function(){ /* ignore */ }) }
    ...
    function connectNow(token){ if(connecting) return; connecting=true; if(ws && ws.readyState===1){connecting=false; return;} banner.style.display='block'; banner.textContent = reconnectAttempts>0? 'Reconnecting…' : 'Connecting…';
      ws = connect(origin, token, function(dir,text,ts,agent){ if(dir==='SYS'){ addSystem(body,text,ts); return;} if(dir==='IN' && lastLocalEcho && text===lastLocalEcho){ lastLocalEcho=''; return;} addMsg(body,dir,text,ts,agent); if(panel.style.display!=='flex' && dir==='OUT'){ unread++; showBadge(); } }, function(){ connecting=false; reconnectAttempts=0; banner.style.display='none'; while(queue.length){ try{ ws.send(queue.shift()) }catch(_){ } } }, function(){ connecting=false; reconnectAttempts++; banner.style.display='block'; var delay = Math.min(10000, 500 * reconnectAttempts); setTimeout(function(){ ensureConn() }, delay) });
    }
