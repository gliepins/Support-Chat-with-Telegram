(function(){
  ...
  function init(opts){style();var origin=((opts.origin||'')||detectOrigin()).replace(/\/$/,'');var store=w.localStorage||{getItem:function(){},setItem:function(){}};
    ...
    continuityCheck();
    function ensureSession(forceNew){ var id=store.getItem('scw:id'); var token=store.getItem('scw:token'); if(!forceNew && id && token){ return Promise.resolve({conversation_id:id, token:token}) } return start(origin,store.getItem('scw:name')||undefined).then(function(s){ store.setItem('scw:origin',origin); store.setItem('scw:id',s.conversation_id); store.setItem('scw:token',s.token); store.setItem('scw:ts', String(Date.now())); return {conversation_id:s.conversation_id, token:s.token} }) }
